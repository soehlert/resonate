...
def test_pipeline_the_national_indie_rock_retention(
    mock_mappers: tuple[TagMapper, TagMapper, TagMapper],
) -> None:
    """Verify track with rock and indie rock retains Indie Rock under primary genre Rock."""
    genre_mapper, subgenre_mapper, mood_mapper = mock_mappers
    provider_mgr = MagicMock(spec=ProviderManager)
    provider_mgr.get_tags_for_track.return_value = (
    ...
    genre_mapper.match_genre_consensus.return_value = [("Rock", "rock", 1.0, 0)]
    subgenre_mapper.match_subgenre_consensus.return_value = [("Indie Rock", "indie rock", 1.0)]
    mood_mapper.match_multiple_tags.return_value = []
    ...
    track = TrackItem(rating_key="32657", title="Runaway", artist="The National")
    result = pipeline.enrich_track(
        track, do_genre=True, do_subgenre=True, do_mood=False, do_bpm=False
    )

    assert result.primary_genre == "Rock"
    assert result.subgenres == ["Indie Rock"]


def test_pipeline_blink_182_punk_promotion(
    mock_mappers: tuple[TagMapper, TagMapper, TagMapper],
) -> None:
    """Verify blink-182 promotes Rock to Punk and retains Alternative Punk."""
    genre_mapper, subgenre_mapper, mood_mapper = mock_mappers
    tags = ["pop rock", "alternative and punk", "rock", "alternative punk", "punk", "rock pop"]
    provider_mgr = MagicMock(spec=ProviderManager)
    provider_mgr.get_tags_for_track.return_value = (tags, tags, True, "blink-182")

    genre_mapper.match_genre_consensus.return_value = [
        ("Rock", "pop rock", 0.95, 0),
        ("Punk", "alternative and punk", 0.91, 1),
        ("Rock", "rock", 0.92, 2),
        ("Punk", "alternative punk", 0.84, 3),
        ("Punk", "punk", 0.84, 4),
        ("Rock", "rock pop", 0.76, 5),
    ]
    subgenre_mapper.match_subgenre_consensus.return_value = [
        ("Pop Rock", "pop rock", 1.874),
        ("Alternative Punk", "alternative punk", 0.96),
    ]
    mood_mapper.match_multiple_tags.return_value = []
    ...
    track = TrackItem(rating_key="29299", title="Stockholm Syndrome", artist="blink-182")
    result = pipeline.enrich_track(
        track, do_genre=True, do_subgenre=True, do_mood=False, do_bpm=False
    )

    assert result.primary_genre == "Punk"
    assert result.subgenres == ["Alternative Punk"]
